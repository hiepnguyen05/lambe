...
import type { StandardService } from '../../types/admin-services.types'

interface AdminReorderServicesModalProps {
...
export function AdminReorderServicesModal({
  category,
  services,
  isOpen,
  isSubmitting,
  onClose,
  onSaveReorder,
}: AdminReorderServicesModalProps) {
  const [orderedItems, setOrderedItems] = useState<StandardService[]>([])

  useEffect(() => {
    if (services) {
      setOrderedItems([...services].sort((a, b) => a.sortOrder - b.sortOrder))
    }
  }, [services, isOpen])
    ...
    <div className="admin-modal-backdrop" onClick={onClose}>
      <div className="admin-modal" style={{ maxWidth: 540 }} onClick={(e) => e.stopPropagation()}>
        <div className="admin-modal__header">
          <h3 className="admin-modal__title">Sắp Xếp Dịch Vụ - {category.name}</h3>
          <button
            type="button"
        ...
        <div className="admin-modal__body">
          <p style={{ fontSize: 13, color: 'var(--color-secondary)', margin: '0 0 12px' }}>
            Sử dụng nút mũi tên để thay đổi thứ tự ưu tiên hiển thị của các dịch vụ trong danh mục <strong>{category.name}</strong>.
          </p>

          {orderedItems.length === 0 ? (
            <p style={{ textAlign: 'center', padding: '24px 0', color: 'var(--color-secondary)' }}>
              Danh mục này chưa có dịch vụ nào để sắp xếp.
            </p>
          ) : (
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                    ...
                    <span className="material-symbols-outlined" style={{ fontSize: 20 }}>
                      {item.iconUrl || 'content_cut'}
                    </span>
                    <div>
                      <div style={{ fontWeight: 600, fontSize: 14 }}>{item.name}</div>
                      <div style={{ fontSize: 12, color: 'var(--color-secondary)' }}>
                        Mã: {item.code} | {new Intl.NumberFormat('vi-VN').format(item.minPriceAmount)}đ - {new Intl.NumberFormat('vi-VN').format(item.maxPriceAmount)}đ
                      </div>
                    </div>
                  </div>
