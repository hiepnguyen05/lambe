...
interface AdminCoverImageModalProps {
...
export function AdminCoverImageModal({
  service,
  isOpen,
  isSubmitting,
  onClose,
  ...
  const currentImage = previewUrl || service.coverImageUrl

  return (
    <div className="admin-modal-backdrop" onClick={onClose}>
      <div className="admin-modal" style={{ maxWidth: 500 }} onClick={(e) => e.stopPropagation()}>
        <div className="admin-modal__header">
          <h3 className="admin-modal__title">Ảnh Bìa Dịch Vụ - {service.name}</h3>
          <button
            type="button"
        ...
        <form onSubmit={handleUploadSubmit}>
          <div className="admin-modal__body">
            ...
            <div style={{ display: 'flex', gap: 10, marginTop: 16 }}>
              ...
              {service.coverImageUrl && !selectedFile && (
                <button
                  type="button"
                  className="admin-cat-btn admin-cat-btn--danger"
                  onClick={() => {
                    if (window.confirm('Bạn có chắc muốn xóa ảnh bìa hiện tại của dịch vụ?')) {
                      void onRemove()
                    }
                  }}
                  disabled={isSubmitting}
